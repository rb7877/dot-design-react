...
export const FilterDropdown = ({ options }: any) => {
  ...
  return (
    <div className={`${cx.filterDropdown}`}>
      <button onClick={opendropdownhandler}>{displayoptions.length > 0 ? displayoptions.map((item: OptionsFilter) => <span className={`${cx.selectedOption}`} onClick={() => { removeSelectedOptions(item) }}>{item.label}</span>) : '--- Select ---'}</button>
